		...
		enroll.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if (response == JOptionPane.YES_OPTION)
				{
					...
					int exists = DatabaseMethods.customerExists(cName.getText(), -1);
					if (exists==-1)
					{
						cName.requestFocus();
						CourseGUI.message("This Customer does not exist!");
						return;
					}
					boolean b = DatabaseMethods.customerAlreadyEnrolled(
							cName.getText(), (String) combo.getSelectedItem());
					if (!b)
					{
						DatabaseMethods.addCourseToCustomer(cName.getText(),
					...
					/*
					 * try { addCourse(cName.getText(), (String)
					 * combo.getSelectedItem());
					 * CourseGUI.message("Course enrollment successful!");
					 * cName.setText(""); combo.setSelectedIndex(0); } catch
					 * (CustomerNotFoundException err) { CourseGUI .message(
					 * "Customer not found or is already enrolled in that course!"
					 * ); }
					 */
				}
			}
